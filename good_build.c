...
char *dm, *dst, *mis_dm, *mis_dst, *source, *obj, *temp_title, *position, *prefix;
/*-------------------------------------*/
Queue *head, *tail;
/*-------------------------------------*/
//void help ();
...
mmaping (char *source)
{
  ...
  sm =
    mmap (0, (long long) statbuf.st_size, PROT_READ, MAP_SHARED | MAP_NORESERVE,
	  src, 0);
...
char *large_load (char *filename)
{
  ...
  (strstr(filename,".fasta")||strstr(filename,".fna")) && ((total_size = get_size(filename)),1) ||  (total_size=(get_size(filename)*2));
...
init_bloom (bloom * bl)
{
  ...
  bloom_init (bl, bl->stat.elements, bl->stat.capacity, bl->stat.e, bl->stat.ideal_hashes, hash, flags);
