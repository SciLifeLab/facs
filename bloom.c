...
int bloom_init(bloom *bloom,BIGNUM size,BIGNUM capacity, float error_rate,int hashes,hash_t hash,int flags)
{
	if (size < 1) {
  ...
  if (capacity == 0 || error_rate == 0) {
	...
	if ((bloom->vector = (char *)malloc(sizeof(char) * ((long long)(bloom->stat.elements / 8) + 1))) == NULL) {
        //printf("-->%d\n",strlen(bloom->vector));
		perror("malloc");
		return -1;
	}
...
int save_bloom (char *filename, bloom *bl, char *prefix)
{
  char *position1;
...
printf("#\n");
printf("# extended from FACS (Fast and Accurate Classification of Sequences)\n");
printf("#\n");
...
printf("DESCRIPTION\n");
printf("\n");
printf("    Bloom filters are a lightweight duplicate detection algorithm proposed\n");
printf("    by Burton Bloom\n");
printf("    (http://portal.acm.org/citation.cfm?id=362692&dl=ACM&coll=portal), with\n");
printf("    applications in stream data processing, among others. Bloom filters are\n");
printf("    a very cool thing. Where occasional false positives are acceptable,\n");
printf("    bloom filters give us the ability to detect duplicates in a fast and\n");
printf("    resource-friendly manner.\n");
printf("\n");
printf("    The allocation of memory for the bit vector is handled in the c layer,\n");
printf("    but perl's oo capability handles the garbage collection. when a\n");
printf("    Bloom::Faster object goes out of scope, the vector pointed to by the c\n");
printf("    structure will be free()d. to manually do this, the DESTROY builtin\n");
printf("    method can be called.\n");
printf("\n");
printf("    A bloom filter perl module is currently avaible on CPAN, but it is\n");
printf("    profoundly slow and cannot handle large vectors. This alternative uses a\n");
printf("    more efficient c library which can handle arbitrarily large vectors (up\n");
printf("    to the maximum size of a long long datatype (at least\n");
printf("    9223372036854775807, on supported systems ).\n");
printf("\n");
printf("    FACS is a novel algorithm, using Bloom filter to accurately and rapidly\n");
printf("    align sequences to a reference sequence. FACS was first optimized and\n");
printf("    validated using a synthetic metagenome dataset. An experimental metagenome\n"); 
printf("    dataset was then used to show that FACS is at least three times faster and\n"); 
printf("    more accurate than BLAT and SSAHA2 in classifying sequences when using\n"); 
...
printf("#   ./simple_check -m 1 -t 0.8 -s 1 -q test.fna -l bloom_list -p test/\n");
printf("#\n");
printf("#   Arguments:\n");
printf("#   -m mode (default 1)\n");
...
printf("#   tolerant_rate is a threshold number between 0 and 1. Any read with a hit score more than tolerant_rate with be considered \n");
printf("#   as captured.\n");
printf("#   -s sampling rate (default 1)\n");
printf("#   sample_rate is a percentage. e.g. 0.2 means you only want to exam 20% reads out of total.\n");
printf("#   -q query file.\n");
printf("#   -l list containing all references files. One file per line\n");  
printf("#   -p prefix (default same path as the script)\n"); 
//printf("#\n");   
...
printf("#   ./simple_remove -m 1 -t 0.8 -q test.fna -l bloom_list -p test/\n");
printf("#\n");
printf("#   Arguments:\n");
printf("#   -m mode (default 1)\n");
