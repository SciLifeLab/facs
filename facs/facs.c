...
static PyMethodDef module_methods[] = {
  ...
  {"remove", facs_bloom_remove, METH_VARARGS | METH_KEYWORDS,
   bloom_docstring},
  {NULL, NULL, 0, NULL}
};
...
facs_bloom_query (PyObject * self, PyObject * args)
{
  ...
  if (!PyArg_ParseTuple
      (args, "ss|dd", &qry, &bloom, &tole_rate, &sampling_rate))
    return NULL;
...
facs_bloom_build (PyObject * self, PyObject * args)
{
  ...
  if (!PyArg_ParseTuple
      (args, "ss|ids", &source, &bloom_filter, &k_mer, &error_rate, &prefix))
    return NULL;
