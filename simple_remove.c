...
/*-------------------------------------*/
char *source, *all_ref, *position, *prefix, *clean, *contam, *clean2, *contam2;
/*-------------------------------------*/
Queue *head, *tail;
/*-------------------------------------*/
bloom *bl_2;
...
int fasta_full_check (bloom *bl, char *begin, char* next);
...
while(p!=next){
    ...
    //printf("here\n");
    //printf("temp_start->%0.30s\n",temp_start);
    //printf("temp_end->%0.30s\n",temp_end);
    //printf("next->%0.30s\n",next);
    if(result ==1){
    #pragma omp critical
    {
...
while (p!=next){
      while (n < k_mer)
	{
	  ...
	  if (bloom_check (bl, key)){
          //printf("hit\n");
          return fasta_full_check(bl,begin,next);
          //return 0;
          } 
          //else
          //printf("unhit\n"); 
...
int fasta_full_check(bloom *bl, char *begin, char* next){
        ...
        int n, m, count_enter, pre_kmer = -1;
...
if (label_m==0 && label_mis ==0)
return -1;
if ((float)label_mis/(float)(label_m+label_mis)<(tole_rate))
return 0;
else
return 1;
